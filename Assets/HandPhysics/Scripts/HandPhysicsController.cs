	...
	*/
    ...
    void FixedUpdate()
    {
        ...
        if (!ObjectAttached)
        {
            if (CheckIfCanAttach())
            {
                if (_objectToAttach != null)
                {
                    AttachObject(_objectToAttach.rigidbody);
                }
            }
        }
        else
        ...
        HandParts[0][0].rigidbody.AddForce(new Vector3(_forearmMovement.x * Time.deltaTime * (ForearmMovementSpeedXZ * 4000), _forearmMovement.y * Time.deltaTime * (ForearmMovementSpeedY * 4000), _forearmMovement.z * Time.deltaTime * (ForearmMovementSpeedXZ * 4000)));
    ...
    void AttachObject(Rigidbody rb)
    {
        foreach (var handPartRoots in HandParts)
        {
            ...
            {
                Physics.IgnoreCollision(handPart.collider, rb.collider);
            }
        }
    ...
    void DetachObject()
    {
        if (_objectToAttach != null)
        {
            foreach (var handPartRoots in HandParts)
            {
                foreach (var handPart in handPartRoots)
                {
                    Physics.IgnoreCollision(handPart.collider, _objectToAttach.collider, false);
                }
            }
        }
